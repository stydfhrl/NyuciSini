    ...
    <div class="card">
      <div class="card-body">
        <div class="d-flex justify-content-between">
          <h2>Tabel Data Laundry</h2>
          <a href="{{url ('data-laundry/create')}}" class="tbl-btn button btn-primary p-2 rounded-2">Add New Laundry</a>
        </div>
          ...
          <table class="table table-hover table-striped border rounded-1">
            <thead>
              <tr>
                ...
                <th class="fw-bold text-center">Jenis Paket</th>
            ...
            <tbody>
              ...
              @foreach ($data as $idx)
                <tr>
                    ...
                    <td class="text-center fs-6">{{$idx -> paketoutlet-> nama}}</td>
                    <td class="text-center fs-6">{{$idx -> nama_paket}}</td>
                    <td class="text-center fs-6">{{$idx -> jenis}}</td>
                    <td class="text-center fs-6">{{$idx -> harga}}</td>
                  {{-- <td class="text-danger">{{$idx ->}}<i class="mdi mdi-arrow-down"></i></td> --}}
                  <td class=" d-flex gap-2 justify-content-center text-center">
                    <a href="{{ url('data-laundry/'.$idx->id.'/edit')}}" class="btn btn-sm fw-semibold text-dark rounded-2 bg-warning"> <i class="fa-solid fa-pen-to-square"></i>
                      Edit
                    </a>
                    <form action="{{ url('data-laundry/'.$idx->id) }}" method="POST">
                      @csrf
                      @method('delete')
